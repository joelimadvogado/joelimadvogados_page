...
export default function PuzzleHero() {
  return (
    <section className="relative w-full min-h-[60vh] md:min-h-[75vh] bg-gradient-to-b from-white to-neutral-50 flex items-start justify-center overflow-visible pt-6 md:pt-10 pb-20 px-6">
      ...
      <div className="hidden md:grid grid-cols-4 gap-[0.05rem] max-w-7xl w-full aspect-[2/1] relative z-10">
        {DESKTOP_ITEMS.map((item, index) => (
          ...
          >
            {/* Glass Card Container - Clean Style */}
            <div className="relative w-full h-full p-[0.025rem] rounded-[1rem] bg-white border border-white/60 shadow-[0_10px_40px_rgba(0,0,0,0.05)] hover:shadow-[0_20px_50px_rgba(4,120,87,0.15)] transition-all duration-500 overflow-hidden">
      ...
      <div className="grid md:hidden grid-cols-2 gap-[0.25rem] w-full aspect-square relative z-10">
        {MOBILE_ITEMS.map((item, index) => (
          <motion.div
            ...
            className="p-[0.1rem] rounded-[1rem] bg-white border border-white/50 shadow-lg"
          >
            <div className="relative w-full h-full rounded-[0.90rem] overflow-hidden">
              <Image
                src={item.src}
                alt={item.alt}
                fill
      ...
      <motion.div
        ...
        className="absolute bottom-[-3rem] md:bottom-[-5rem] z-30 flex flex-col items-center"
      >
        <div className="bg-white/90 backdrop-blur-xl text-secondary px-12 md:px-20 py-8 md:py-10 shadow-[0_30px_60px_rgba(30,58,138,0.15)] text-center border-t-4 border-primary rounded-2xl border-b border-white/50">
          ...
          <p className="text-[10px] md:text-xs tracking-[0.3em] font-bold text-gray-500 uppercase flex items-center justify-center gap-4">
            Direito Civil <span className="w-1.5 h-1.5 rounded-full bg-primary"></span> Direito Previdenciário
          </p>
        </div>
      </motion.div>
    </section>
